...
* 词根字典: <https://www.youdict.com/root/id/25>
...
a / a+辅音 开头=> 语气词，一般是加强语气(等同于 to 加强语气)
...
ad => d可以省略
    => avenue 大道  a(ad) + ven(come 来) + ue(名词后缀)
    => avail 利用 a(ad) + vail(=val 强壮、森林发展，才能留下来，就有价值的)
                      prevail(盛行、流行、占优势)
...
an  => 否定
    => anonyous 无政府主义的  an(否定) + onym(nym - name 名字) + ous(形容词后缀) 
...
ap  => to
    => apparatus 仪器、设备、装置（辅助的作用） ap(to 加强语气) + par(设备需要人来 准备 - para) + atus(复合名词后缀) 
...
ass  => 屁股
    => assimilate （变态的）暗杀 ass(屁股) + ass(屁股) + in + ate(吃 eat过去式) 
...
e / ex / e+辅音 开头 => 向外
e =>
    => elaborate 精心制作的(付出很多精力做出来的东西) e(向外) + labor(劳动) + ate(形容词)
ex =>
    => exit 出口、推出
    => export 港口
...
en => 使、让
    => enclosure 围住、围栏 en + clos(关闭 close) + ure(名词后缀)
...
bene => 好 (benefit)
    => beneficial 有益的，有利的  bene + fic(做  fiction 小说使人虚构，做出来的) + ial(形容词后缀)
...
cand => 白色
     ...
     => candidate 候选人  cand + id(形容词后缀) + ate(特殊情况-名词后缀)
         graduate 毕业、毕业生
...
calc/calcul => 石头 (chalk 粉笔)
    => caleium 钙 calc + ium(名词后缀)
    => calculate 计算(一天一个小石头，用于计数)  calcul + ate
    => calcify 钙化，使成石灰  calcul + ify(动词后缀)
...
cate => 向下
    => catelogue 小册子，目录(在下面给大家说的话) cate(下面) + log(作为词根：说；如果单词：日志) + ue(名词后缀)
...
cert => sure 确定的
  => cartficate 证书 cert(certain 一定确定，表示sure) + fic(做  fiction 小说使人虚构，做出来的) + ate(名词后缀) 
...
con =>
    => consonant 协调一致的(所有的声音汇集到一起) con + son(声音) + ant(形容词后缀)
